...
const Header = ({colorScheme, toggleColorScheme}: TProps): JSX.Element => {
  ...
  const LogoLink = (): JSX.Element => (
    <Link className={classes.logo_container} href="/" passHref>
      <Text style={{fontSize: '120%'}} className={classes.logo}>
        Portfolio
      </Text>
    </Link>
  );
    ...
    <MHeader height={70} p="xl" className={classes.header}>
      <Box className={classes.content_container}>
        <Box className={classes.menu_container}>
          {smallerThan481 ? (
            <Menu trigger="hover" delay={200} className={classes.menu}>
              <Menu.Label>Pages:</Menu.Label>
              {links.map(
                (el, i) =>
                  !el.show && (
                    <Link key={i} href={el.href} passHref>
        ...
        <Box className={classes.links_container}>
          {breakpointLinks.length > 0 &&
            breakpointLinks.map(
              (el, i) =>
                el.show && (
                  <Text key={i} className={classes.link} variant="link" component="a" href={el.href}>
                    {el.text}
                  </Text>
                ),
            )}
        ...
        <ActionIcon
          className={globalClasses.action_icon}
